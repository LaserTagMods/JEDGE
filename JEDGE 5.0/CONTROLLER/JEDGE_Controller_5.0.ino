  ...
  <div class="content">
    <div class="card">
      ...
      <p><select name="presetgamemodes" id="presetgamemodesid">
        ...
        <option value="2159">Trouble in Terrorist Town</option>
        </select>
      </p>
      <h2>Lives</h2>
      ...
      <p><button id="standby" class="button">Standby Mode</button></p>
    </div>
  </div>    
  <div class="stopnav">
  ...
  <div class="content">
    <div class="card">
      ...
      <p><select name="playercount" id="playercountid">
        ...
        <option value="2001">20</option>
        <option value="2002">30</option>
        <option value="2003">40</option>
        <option value="2004">50</option>
        <option value="2005">60</option>
        <option value="2006">64</option>
        </select>
      </p>
      <h2>Tagger Mode</h2>
      <p><select name="playermode" id="playermodeid">
        <option value="2007">Standard JEDGE Player</option>
        <option value="2008">Auto Start - Strong</option>
        <option value="2009">Auto Start - Stronger</option>
        <option value="2010">Auto Start - Strongest</option>
        <option value="2011">Auto Start - Ridiculous</option>
        </select>
      </p>
      <h2>Tagger Gen</h2>
      <p><select name="genassignment" id="genassignmentid">
        <option value="1899">Select Gun Generation</option>
        <option value="1971">Gen 1 Hardwired</option>
        <option value="1972">Gen 2 Hardwired</option>
        <option value="1973">Gen 3 Hardwired</option>
        <option value="1974">Gen 1 Bluetooth</option>
        <option value="1975">Gen 2 Bluetooth</option>
        <option value="1976">Gen 3 Bluetooth</option>
        <option value="1977">Gen 1 HC-05</option>
        </select>
      </p>
      <form action="/get">
      ...
      <form action="/get">
        Bluetooth Name: <input type="text" name="input3">
        <input type="submit" value="Submit">
      </form><br>
      <h2>For Gen1 HC-05 Serial#, input as (1234,56,abcdef)</h2>
...
if (!!window.EventSource) {
 ...
 source.addEventListener('new_readings', function(e) {
  ...
  function initButton() {
    ...
    document.getElementById('standby').addEventListener('click', togglestandby);
    ...
    document.getElementById('playermodeid').addEventListener('change', handleplayermode, false);
  ...
  function handleplayermode() {
    var vr = document.getElementById("playermodeid").value;
    websocket.send(vr);
  }
...
void handleWebSocketMessage(void *arg, uint8_t *data, size_t len) {
  ...
  if (info->final && info->index == 0 && info->len == len && info->opcode == WS_TEXT) {
    ...
    if (strcmp((char*)data, "togglestandby") == 0) { // standbymode
      Menu[9] = 902;
      ledState = !ledState;
      notifyClients();
    ...
    if (strcmp((char*)data, "togglepbend") == 0) { // game end
      Menu[14] = 2196;
      ledState = !ledState;
      notifyClients();
      Serial.println("menu = " + String(Menu[14]));
    ...
    if (strcmp((char*)data, "1974") == 0) {
      ledState = !ledState;
      notifyClients();
      Menu[0] = 1974;
      PrimaryMenu();
    }
    if (strcmp((char*)data, "1975") == 0) {
      ledState = !ledState;
      notifyClients();
      Menu[0] = 1975;
      PrimaryMenu();
    }
    if (strcmp((char*)data, "1976") == 0) {
      ledState = !ledState;
      notifyClients();
      Menu[0] = 1976;
      PrimaryMenu();
    }
    if (strcmp((char*)data, "1977") == 0) {
      ledState = !ledState;
      notifyClients();
      Menu[0] = 1976;
      PrimaryMenu();
    ...
    if (strcmp((char*)data, "2007") == 0) {
      ...
      Menu[0] = 2007;
      PrimaryMenu();
      Serial.println("Player mode = Standard Jedge Player");
    }
    if (strcmp((char*)data, "2008") == 0) {
      ...
      Menu[0] = 2008;
      PrimaryMenu();
      Serial.println("Player mode = Auto Strong");
    }
    if (strcmp((char*)data, "2009") == 0) {
      ...
      Menu[0] = 2009;
      PrimaryMenu();
      Serial.println("Player mode = Auto stronger");
    }
    if (strcmp((char*)data, "2010") == 0) {
      ...
      Menu[0] = 2010;
      PrimaryMenu();
      Serial.println("Player mode = Auto strongest");
    }
    if (strcmp((char*)data, "2011") == 0) {
      ...
      Menu[0] = 2011;
      PrimaryMenu();
      Serial.println("Player mode = Auto ridiculous");
    }
    ...
    if (strcmp((char*)data, "2159") == 0) {
      ...
      notifyClients();
      Menu[0] = 2159;
      // trouble in terrorist town, need to run a process to assign terrorist and put players into free for all mode with one life
      // first, lets see who is all playing so we know what numbers we can use to assign as terrorists and how many players there are:
      byte TTTPlayercount = 0;
      byte ProcessCounter = 0;
      while (ProcessCounter < TaggersOwned) {
        // ping players via espnow
        // send a packet to each player and see if they respond back
